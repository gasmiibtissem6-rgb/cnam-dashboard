...
export default function LoginPage() {
  ...
  const [needsPasswordChangeAfter2FA, setNeedsPasswordChangeAfter2FA] = useState(false);
  ...
  const checkUserSecurityStatus = async (userId: string) => {
    ...
    const passwordAgeInDays = settings?.password_changed_at
      ? (Date.now() - new Date(settings.password_changed_at).getTime()) / (1000 * 60 * 60 * 24)
      : 0;

    const requiresPasswordChange = !!(
      settings?.password_must_change ||
      (settings?.password_changed_at && passwordAgeInDays > 30)
    );

    // Check if user dismissed the reminder recently (within 24h)
    const dismissedAt = localStorage.getItem(`pwd_remind_later_${userId}`);
    const wasDismissedRecently = dismissedAt && (Date.now() - parseInt(dismissedAt)) < 24 * 60 * 60 * 1000;

    const shouldShowPasswordChange = requiresPasswordChange && !wasDismissedRecently;

    return { requires2FA: true, requiresPasswordChange, shouldShowPasswordChange };
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { requires2FA, shouldShowPasswordChange } = await checkUserSecurityStatus(currentUser.id);

    setNeedsPasswordChangeAfter2FA(shouldShowPasswordChange);
    setShow2FA(true);
    setIsSubmitting(false);
  };
  ...
  const handle2FAVerified = async () => {
    ...
    if (needsPasswordChangeAfter2FA) {
    ...
    <>
        ...
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-xl">
              <img src={cnamIcon} alt="CNAM" className="h-20 w-20" />
            </div>
      ...
      <ChangePasswordDialog
        ...
        onRemindLater={async () => {
          const { data: { user: currentUser } } = await supabase.auth.getUser();
          if (currentUser) {
            localStorage.setItem(`pwd_remind_later_${currentUser.id}`, Date.now().toString());
          }
          setShowForcePasswordChange(false);
          setPendingSecurityCheck(false);
          await logSecurityEvent("login_success", "low", { email });
