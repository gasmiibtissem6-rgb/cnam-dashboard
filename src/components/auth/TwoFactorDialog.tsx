...
export function TwoFactorDialog({ open, onVerified, userEmail }: TwoFactorDialogProps) {
  ...
  const loadPendingSetup = async () => {
    try {
      ...
      throw new Error("QR code indisponible pour le moment");
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-md [&>button.absolute]:hidden" onInteractOutside={(e) => e.preventDefault()}>
              ...
              {showQrCode ? (
                ...
                <>
                  ...
                  <div className="flex flex-col items-center gap-3 py-4">
                    ...
                    <p className="text-sm text-muted-foreground">Chargement du QR code...</p>
                    <Button type="button" variant="ghost" onClick={() => setShowQrCode(false)}>Retour</Button>
                  </div>
                )
              ) : (
                <>
